...
function timeSorter(v) {
    if (v > 0) {
        ...
        const seconds = Math.trunc(calcSeconds);
...
document.addEventListener("click", e => {
  ...
  const throttleProgressBar = throttle(() => {
    document.querySelectorAll(".stv-hp-progress-bar").forEach(calculateProgressBar)
  }, 250)
  ...
  function calculateProgressBar(progressBar) {
    ...
    const slider = progressBar.closest(".stv-hp-row").querySelector(".stv-hp-slider")
  ...
  function onHandleClick(handle) {
    const progressBar = handle.closest(".stv-hp-row").querySelector(".stv-hp-progress-bar")
    const slider = handle.closest(".stv-hp-container").querySelector(".stv-hp-slider")
